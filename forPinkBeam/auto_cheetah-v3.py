...
def parse_cmdline_args():
    ...
    parser.add_argument('-r','--r', default=False, action='store_true', help='Force to rerun blocks that you want')
...
def processing(path, rerun):
    if 'darks' not in path and 'todel' not in path:
        ...
        if len(h5files) > 0:
            ...
            elif len(glob.glob(os.path.join(new_output_path, '*h5'))) > 0 and not(rerun):
                return
