...
if __name__ == "__main__":
    ...
    if args.s is not None:
        ...
        if args.f is None and args.pt is None:
            ...
            if len(files) == 0:
                files = glob.glob(os.path.join(args.i,'*.h5')) + glob.glob(os.path.join(args.i,'*.nx5'))
                if len(files) == 0:
        ...
        if args.f is None and args.pt is not None:
            ...
            if len(files) == 0:
                files = glob.glob(os.path.join(args.i,f'*{args.pt}*h5')) + glob.glob(os.path.join(args.i,f'*{args.pt}*nx5'))
                if len(files) == 0:
                    print(f'Check {args.i} - something went wrong. Carefully check format and filenames')
                    correct = False
                else:
