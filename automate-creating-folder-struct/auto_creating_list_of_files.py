...
def creating_lists(name_of_blocks):
    ...
    with open(filenameLST,'w+') as files_lst:
        for block in blocks:
            ...
            files_of_interest = [os.path.join(dir_from, filename) for filename in data_files if (file_extension in filename and pattern in filename and not('sum' in filename))]
