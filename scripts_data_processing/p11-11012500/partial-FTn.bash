...
command="$ambigator $stream.stream -y ${PGY2} -w ${PGW2} --ncorr=15000 -n 50 -j 128 --fg-graph=fg_6m1.dat -o amb-$stream-temp2.stream #--lowres=20 --highres=2 "
#echo $command >> $SLURMFILE                         
	...
	command="$partialator --no-logs --iterations=$iterations --model=$model "$minres" "$push" -i $stream.stream  -o $data.hkl -y ${PGY2} -j 80"
    echo $command >> $SLURMFILE
