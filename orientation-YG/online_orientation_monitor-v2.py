...
class CellExplorerGui(QtWidgets.QMainWindow):
    ...
    def __init__(self, stream_filename):
        ...
        self._sp1 = gl.GLScatterPlotItem(pos=np.array([0,0,0]),color=(1.0,0,0,0.5), size=markerSize, pxMode=False)
        ...
        self._sp2 = gl.GLScatterPlotItem(pos=[], color=(0.,1.0,0,0.5), size=markerSize, pxMode=False)
        ...
        self._sp3 = gl.GLScatterPlotItem(pos=[], color=(1.0,0.0,1.0,0.5), size=markerSize, pxMode=False)
        ...
        self._bstar_widget.setSizePolicy(self._cstar_widget.sizePolicy())
    ...
    def _update_gui(self):
        ...
        self._delay_widget.setText("Last indexed event: {} minutes {:.3f} seconds ago".format(delay_min, delay_sec))        
        
        astar_values = np.array([data["astar"][0] for data in self._data.values()])
        self._sp1.setData(pos=astar_values,color=(1.0,0,0,0.5),size=markerSize, pxMode=False)
        
        bstar_values = np.array([data["bstar"][0] for data in self._data.values()])
        self._sp2.setData(pos=bstar_values,color=(0.,1.0,0,0.5),size=markerSize, pxMode=False)
        
        cstar_values = np.array([data["cstar"][0] for data in self._data.values()])
        self._sp3.setData(pos=cstar_values,color=(1.0,0.0,1.0,0.5),size=markerSize, pxMode=False)
