...
class Raster(QtGui.QPolygonF):
    ...
    stepsizeHorizontal = 0.0 # horizontal stepsize in pixels
    stepsizeVertical = 0.0   # vertical stepsize in pixels
    ...
    colours = []             # set of colours for each position           
    ...
    def setColours(self, colours):
        if type(colours) == list and len(colours) == len(self.positions):
            self.colours = colours

    def getPositionsPerRow(self):
        if (not self.positionsValid):
            self.calculateScanPositions()
        return self.positionsPerRow

    def getPositionsPerRowTriangular(self):
        if (not self.positionsValid):
            self.calculateScanPositions()
        return self.positionsPerRowTriangular

    def getPositionsPerColumn(self):
        if (not self.positionsValid):
            self.calculateScanPositions()
        return self.positionsPerColumn
    ...
    def getScanPositions(self):
        ...
        for pos in self.positions:
            #positions.append((pos - self.pixmapCenter) / self.conversion + self.offset)
            x = pos.x() - self.pixmapCenter.x()
            y = -(pos.y() - self.pixmapCenter.y())
            positions.append(QtCore.QPointF(x, y) / self.conversion + self.offset)
    ...
    def getScanPositionSymbols3(self):
        """Returns a QGraphicsPixmapItem with ellipses for all scan points.
        ...
        ellipse = QtGui.QPixmap(beamsizeHorizontal, beamsizeVertical)
        ellipse.fill(Qt.transparent)
        painter = QtGui.QPainter(ellipse)
        painter.setPen(QtGui.QPen(QtGui.QColor(Qt.green), 1, Qt.SolidLine))
        painter.setBrush(Qt.green)
        painter.setOpacity(0.2)
        painter.drawEllipse(0, 0, beamsizeHorizontal, beamsizeVertical)
        painter.end()
        offsetX = min(self.topLeft.x(), self.bottomLeft.x())
        offsetY = min(self.topLeft.y(), self.topRight.y())
        width = max(self.topRight.x(), self.bottomRight.x()) - offsetX
        height = max(self.bottomLeft.y(), self.bottomRight.y()) - offsetY
        symbols = QtGui.QPixmap(width, height)
        symbols.fill(Qt.transparent)
        painter2 = QtGui.QPainter(symbols)
        textsymbols = QtWidgets.QGraphicsItemGroup()
        i=0
        for pos in self.positions:
            painter2.drawPixmap(pos.x()-offsetX-beamsizeHorizontal/2, pos.y()-offsetY-beamsizeVertical/2, ellipse)
            ...
            textsymbols.addToGroup(text)
            i += 1
            
        painter2.end()
        symbols = QtGui.QGraphicsPixmapItem(symbols)
        symbols.setOffset(offsetX, offsetY)
        return symbols
    ...
    def getScanPositionSymbols(self):
        ...
        beamsizeHorizontal = self.conversion * self.beamsizeHorizontal
        beamsizeVertical = self.conversion * self.beamsizeVertical
        symbols = QtWidgets.QGraphicsItemGroup()
        i = 0
        for pos in self.positions:
            ...
            if (len(self.colours)==len(self.positions)):
                color = QtGui.QColor(self.colours[i]) #color = QtGui.QColor(0, 0, 204)
    ...
    def calculateScanPositions(self):
        ...
        stepsizeVertical = self.conversion * self.stepsizeVertical
        if(self.scanType & Raster.RASTER_SCAN_TRIANGULAR):
            if(self.scanType & Raster.RASTER_SCAN_VERTICAL):
                ...
                #stepsizeVertical = self.conversion * sqrt(3) * 0.5 * self.stepsizeHorizontal
                stepsizeVertical = self.conversion * self.stepsizeHorizontal
        width = sqrt(abs((self.topRight.x() - self.topLeft.x())**2 + (self.topRight.y() - self.topLeft.y())**2))
        height = sqrt(abs((self.bottomLeft.x() - self.topLeft.x())**2 + (self.bottomLeft.y() - self.topLeft.y())**2))
        #print "width",width,"stepsizeHorizontal",stepsizeHorizontal
        if isnan(width): width = 0.0
        if isnan(height): height = 0.0
        
        stepsHorizontal = int(floor(width / stepsizeHorizontal))
        stepsVertical = int(floor(height / stepsizeVertical))
        ...
        running_through_stepsHorizontal = []
        running_through_stepsVertical = []
        
        try: #galchenm
            with open('joined.lst', 'r') as file:
                for line in file:
                    X,Y = line.strip().split(',')
                    running_through_stepsHorizontal.append(int(X))
                    running_through_stepsVertical.append(int(Y))
        except IOError:
        ...
        posistionsVertical = []
        positionsTriangular = []
        if len(running_through_stepsVertical) > 0:
            print('We are on the line 971 - we have already fly scan')
            #for x in range(stepsHorizontal): before
            for x in running_through_stepsHorizontal:
                posistionsHorizontal.append(self.unitVectorX * ((x + 0.5) * stepsizeHorizontal + stepOffsetHorizontal))
            #for y in range(stepsVertical): before
            for y in running_through_stepsVertical:
                posistionsVertical.append(self.unitVectorY * ((y + 0.5) * stepsizeVertical + stepOffsetVertical))
        else:
            print('We are on the line 979 - we do not have fly scan')
        ...
        if(self.scanType & Raster.RASTER_SCAN_TRIANGULAR):
            ...
            if(self.scanType & Raster.RASTER_SCAN_VERTICAL):
                for y in range(stepsVertical - 1):
                    positionsTriangular.append(self.unitVectorY * ((y + 0.5) * stepsizeVertical + stepOffsetVertical + stepOffsetTriangular))
            else:
        ...
        if(self.scanType & Raster.RASTER_SCAN_VERTICAL):
            ...
            # Definition of self.raws was removed from calculateScanPositions() for new version
            # Since dataCollection needs these function, I(Pavel) copied all necessary code into this function 
            i = 0
            self.rows = []
            for vert in posistionsVertical:
                self.rows.append([])
                
                #if ((i%2)==1): # do always SCAN_SNAKE
                #    posistionsHorizontal.reverse()
                ...
                #    posistionsHorizontal.reverse()
                i += 1
        self.positionsPerRow = len(posistionsHorizontal)
        self.positionsPerRowTriangular = len(positionsTriangular)
        self.positionsPerColumn = len(posistionsVertical)
...
class RasterItem(QtWidgets.QGraphicsItemGroup):
    ...
    def itemChange(self, change, scene):
    ...
    def move(self, distance, um=True, keepScanPosition=False):
        ...
        if(self.rasterGroup is not None):
            offset = self.rasterGroup.offset()
            if(um):
                distanceUm = distance
                distance *= self.conversion.getValue()
            self.rasterGroup.setOffset(distance - offset)
        #self.updateRaster()
        self.emitValuesChanged(oldRaster)
