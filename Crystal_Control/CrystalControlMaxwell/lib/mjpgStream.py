...
#PP oopps following strings make some things simpler, at least it works without sending fancy comands to plagin        
    ...
    def run(self):
        ...
        while(self.running):
            ...
            if(frame):
                ...
                #frame = frame.transformed(self.rotation)
                self.frame = frame
    ...
    def stop(self):
        ...
        """
        self.running = False
        self.wait(500) # waits until run stops on its own  

    def httpGet(self, query, host=None, port=None, path=None, decode="byte"):
        ...
        httpClient = http.client.HTTPConnection(host, port, timeout=3)
        try:
    ...
    def getControls(self, plugin=None, dest=None):
        ...
        if(query is not None):
            data = self.httpGet(query, decode="utf-8")
            if(data is not None):
    ...
    def setImageSize(self, width=0, height=0, aspectRatio=Qt.KeepAspectRatioByExpanding, transformMode=Qt.FastTransformation):
