...
class Path(QObject):
    ...
    def set_run_number(self, number):
        ...
        if self.number != number:
            ...
            if self.debug:
                print("Path.set_run_number() " +
                        "set to \"" + str(self.number) + "\"")
        elif self.debug:
            print("Path.set_run_number() " +
                    "stays at \"" + str(self.number) + "\"")
    ...
    def set_run_number_check(self, template):
        ...
        if number != self.get_run_number():
            self.value_changed.emit("number")
        if self.debug:
            print("Path.set_run_number_check() " +
        ...
        if force:
            ...
            for d in dirs:
                ...
                if not os.access(current_dir, os.F_OK):
                    try:
                        os.mkdir(current_dir, mode=0o777)
                    except:
                        return None
        return path
