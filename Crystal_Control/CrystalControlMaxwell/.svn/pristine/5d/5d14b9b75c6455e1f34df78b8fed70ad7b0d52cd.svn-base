...
class Filesystem:
    ...
    def getPath(self, pathType=None, force=False):
        ...
        if pathType + 1 <= self.FS_MASK_TYPE and pathType in self.cache:
            if(force):
        ...
        if type != self.FS_TYPE_NONE:
            #user
            if self.user != "":
                path += self.user
            #sample
            path += self.sample + self.sample
            if self.types[int(type/16)] != "":
                path += "_" + self.types[int(type/16)]
            path += "_%03d"%self.runnumber
            # no subfolders per scanpoint any longer
    ...
    def getFilename(self, type=None, suffix="_master.h5"):
        if type is None:
            type = self.FS_TYPE_REGULAR
        type = type & self.FS_MASK_TYPE
        name = self.sample.lstrip("/")
        if self.types[int(type/16)] != "":
            name += "_" + self.types[int(type/16)]
        name += "_%03d"%self.runnumber
        #no posnumbers with the Eiger anymore
        #if type == self.FS_TYPE_SCAN:
    ...
    def getOut(self, type=None):
        path = self.getPath(type, force=False)
        depth = path.count("/")
        return depth * "../"
    ...
    def setSample(self, sample):
        ...
        dirs = sample.strip("/").split("/")
        if(len(dirs) < 1 or dirs[0] == ""):
            dirs = [self.default.lstrip("/")]
        sample = "/" + dirs[0]
        if(sample.find("*") > -1 or sample.find("?") > -1 or sample.find("..") > -1):
            return False
    ...
    def setUserAndSample(self, path):
        path = str(path).replace("\\", "/")
        dirs = path.strip("/").rsplit("/", 1)
        if(len(dirs) > 1):
            self.setUser(dirs[0])
            self.setSample(dirs[1])
        else:
            self.setUser("")
            self.setSample(dirs[0])
    ...
    def writeFile(self, path, content):
        try:
            ...
            print("open file failed:", str(sys.exc_info()))
            return False
