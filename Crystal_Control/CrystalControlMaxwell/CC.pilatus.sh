...
if [ "$i" -ge "0" ] 2>/dev/null
 ...
 then 
  ...
  echo 
  echo  " " >> /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_${i}.log
  echo "Starting Logging CrystalControl with Beamtime ID: ${i}" >> /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_${i}.log
  echo  " " >> /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_${i}.log
...
#python -u /gpfs/local/shared/CFEL/p11/crystalControl.py 2>&1 | while read line; do echo `date +"%A %d-%m-%Y  %T  (KW %V) "` ":" $line; done | tee -a /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_${i}.log
  ...
  echo 
  echo  >> /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_local.log
  echo "Starting Logging CrystalControl with Beamtime ID: local" >> /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_local.log
  echo  >> /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_local.log
...
#python -u /gpfs/local/shared/CFEL/p11/crystalControl.py 2>&1 | while read line; do echo `date +"%A %d-%m-%Y  %T  (KW %V) "` ":" $line; done | tee -a /gpfs/local/shared/CrystalControl/CC_Log/CC_${d}_local.log
