...
for key in data.keys():
    try:
        name = os.path.basename(key.split('//')[0].strip()) #
...
for run in d:
    ...
    mu, std = norm.fit(ddata)
    ddata=ddata[ddata<500]
    ...
    #plt.hist(ddata, bins=700, histtype=u'step', label="{}".format(run))
    plt.hist(ddata, bins=75, density=True, histtype=u'step', label="{}".format(run))
    #xmin, xmax = plt.xlim()
