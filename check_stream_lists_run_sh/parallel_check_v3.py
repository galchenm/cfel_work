...
"""
python3 parallel_check_v3.py [user] [path from] [fraction]


Ex.,
python3 parallel_check_v3.py  galchenm /asap3/petra3/gpfs/p11/2020/data/11009046/scratch_cc/galchenm/processed/blocks 0.9
"""
...
def sh_call_old(di):
    ...
    if len(files_lst) == 0:
        ...
        for s in files_lst:
            dic_list[os.path.join(path_from, os.path.basename(s).replace('split-events-','').replace('EV-','').split(".")[0])] = s
...
def main_func(user, path_from, fraction):
    ...
    process = subprocess.Popen(shlex.split(command), stdout=subprocess.PIPE)
    runs = process.communicate()[0].decode('utf-8').split('\n')
    
    if len(runs) <= 5:
        with Pool(processes=20) as pool:
            pool.map(sh_call, d_runs)
...
if __name__ == "__main__":
    ...
    while True:
        main_func(user, path_from, fraction)
