...
def parse_cmdline_args():
    ...
    parser.add_argument('-maxres','--maxres', type=int, help="Use this flag if you want to force use --max-resolution") 
    parser.add_argument('-snr','--snr', type=int, help="Use this flag if you want to force use --min-snr")
    parser.add_argument('-min_num_peaks_for_hit','--min_num_peaks_for_hit', type=int, help="Use this flag if you want to force use --min-num-peaks-for-hit")   
    parser.add_argument('-max_num_peaks_for_hit','--max_num_peaks_for_hit', type=int, help="Use this flag if you want to force use --max-num-peaks-for-hit")
    ...
    parser.add_argument('--cc', default=False, action='store_true', help="Use this flag if you want to estimate resolution according to CCstar <= 0.5")
    parser.add_argument('--ccrs', default=False, action='store_true', help="Use this flag if you want to estimate resolution according to the intersection between Rsplit and 100*CCstar")
...
def filling_template(stream_filename, block_name, output, bin_size, min_good_pix_count, max_res, min_snr, min_num_peaks_for_hit, max_num_peaks_for_hit):
    os.chdir(output)
    try:
        command = f'grep -e indexamajig {stream_filename}'
        result = subprocess.check_output(shlex.split(command)).decode('utf-8').strip().split('\n')[0]
        ...
        if min_num_peaks_for_hit is None:
        ...
        if max_num_peaks_for_hit is None:
        ...
        if min_snr is None:
            ...
            minimum_snr = float(minimum_snr[0].split('=')[-1]) + 0.5 if len(minimum_snr)>0 else 4.5
        else:
            minimum_snr = min_snr
...
def running_main(blocks_filename, raw_data_path, processed_raw_data_path, output_path, bin_size, min_good_pix_count, forces_template, skip_flag, max_res_value, min_snr, min_num_peaks_for_hit, max_num_peaks_for_hit, ccstar_only_estimation, rsplit_ccstar_estimation):
    logger = logging.getLogger('app')
    ...
    if forces_template is None:
        ...
        for key in d_info:
            ...
            if max_res_value is not None:
                template_name = filling_template(stream_filename, key, templates_folder, bin_size, min_good_pix_count, max_res_value, min_snr, min_num_peaks_for_hit, max_num_peaks_for_hit)
                runs = d_info[key]['blocks']
                for run in runs:
                    binning(run, raw_data_path, output_path, template_name, skip_flag)
            else:
                
                if ccstar_only_estimation:
                    ...
                    CCstar_dat_file = glob.glob(f'{folder}/j_stream/*CCstar.dat', recursive=False)[0] if len(glob.glob(f'{folder}/j_stream/*CCstar.dat', recursive=False)) > 0 else ''#CCstar dat file for estimation max res for hits finding
                                
                    data_resolution = calculating_max_res_from_CCstar_dat(CCstar_dat_file) if len(CCstar_dat_file) > 0 else -1000
                elif rsplit_ccstar_estimation:
                    
                    #CCstar_dat_file = glob.glob(f'{folder}/**/*CCstar.dat', recursive=True)[0] if len(glob.glob(f'{folder}/**/*CCstar.dat', recursive=True)) > 0 else ''#CCstar dat file for estimation max res for hits finding
                    CCstar_dat_file = glob.glob(f'{folder}/j_stream/*CCstar.dat', recursive=False)[0] if len(glob.glob(f'{folder}/j_stream/*CCstar.dat', recursive=False)) > 0 else ''#CCstar dat file for estimation max res for hits finding
                    Rsplit_dat_file = glob.glob(f'{folder}/j_stream/*Rsplit.dat', recursive=False)[0] if len(glob.glob(f'{folder}/j_stream/*Rsplit.dat', recursive=False)) > 0 else ''#Rsplit dat file for estimation max res for hits finding
                ...
                if data_resolution != -1000:
                    ...
                    template_name = filling_template(stream_filename, key, templates_folder, bin_size, min_good_pix_count, max_res, min_snr, min_num_peaks_for_hit, max_num_peaks_for_hit)
                    runs = d_info[key]['blocks']
                    for run in runs:
                        binning(run, raw_data_path, output_path, template_name, skip_flag)
                else:
                    #print(f'Warning: Check {dat_file}')
                    logger.info(f'Warning: Check {key}; it was run with max-res=10000000000')
                    max_res = 10000000000
...
if __name__ == "__main__":
    ...
    min_snr = args.snr
    min_num_peaks_for_hit = args.min_num_peaks_for_hit
    max_num_peaks_for_hit = args.max_num_peaks_for_hit
    ccstar_only_estimation = args.cc
    rsplit_ccstar_estimation = args.ccrs
    
    if not(os.path.exists(output_path)):
        os.mkdir(output_path)
    
    running_main(blocks_filename, raw_data_path, processed_raw_data_path, output_path, bin_size, min_good_pix_count, forces_template, skip_flag, max_res_value, min_snr, min_num_peaks_for_hit, max_num_peaks_for_hit, ccstar_only_estimation, rsplit_ccstar_estimation)
    print('Finished')