...
def power2(I):
    ...
    I = np.where( (len_Ib < 8+k) | ((len_Ib == 8+k) & (Ib_3plk == 0)), 0, (((-1) ** k) * (2 ** (len_Ib - 3 - k + Ib_3plk))))
    return I
...
def power2_upt(I):
    ...
    tmpI = np.where( (len_Ib < 9+k) , 0, ((-1) ** k) * ((2. ** (len_Ib - 3 - k)) * Ib_2plk + (2. ** (len_Ib - 4 - k)) * Ib_3plk + (2. ** (len_Ib - 5 - k )) * Ib_4plk) + (2. ** (len_Ib - 5 - k)) * Ib_5plk)
...
with h5.File(file_cxi, 'a') as f:
    ...
    d[0,] = power2(data[0,]).astype(np.int32) #(round_sqrt(data[0,].astype(np.int32)))
    
    for i in range(num):
        d.resize((i+1,) + shape_data)
        d[i,] = power2(data[i,]).astype(np.int32) #(round_sqrt(data[i,].astype(np.int32)))