...
def parse_cmdline_args():
    ...
    parser.add_argument('-n','--n',type=int, help="Treshold for resolution")
...
def sqrt_compression_function(file):
    job_file = file.split('.')[0] + '.sh'
    with open(job_file, 'w+') as fh:
        fh.writelines("#!/bin/sh\n")
        fh.writelines("#SBATCH --job=%s.job\n" % file.split('.')[0])
        ...
        fh.writelines("#SBATCH --mem=500000\n")
        fh.writelines("module load anaconda3/5.2\n")
        command = f'python3 /gpfs/cfel/group/cxi/scratch/2020/EXFEL-2019-Schmidt-Mar-p002450/scratch/galchenm/scripts_for_work/binningANDcompression/upt_programs/rounding/sqrt_compression.py {file} {path_to_data_cxi} {path_to_data}'
        fh.writelines(command)
    os.system("sbatch %s" % job_file)
        ...
        for dir in dirs:
            ...
            if 'S000' not in dir:
                ...
                if len(files) > 0:
                    ...
                    if args.sqrt:
                        for file in files:
                            sqrt_compression_function(file)
                    if args.log:
                        for file in files:
