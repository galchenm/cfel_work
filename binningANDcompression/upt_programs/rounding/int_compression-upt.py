...
with h5.File(out_file, mode) as f:
    ...
    d[0,] = (np.round(data[0,]/N,0)*N).astype(np.int32) if lim_up is None else np.where(data[0,] < lim_up, (np.round(data[0,]/N,0)*N).astype(np.int32), lim_up)
    
    for i in range(num):
        d.resize((i+1,) + shape_data)
        d[i,] = (np.round(data[i,]/N,0)*N).astype(np.int32) if lim_up is None else np.where(data[i,] < lim_up, (np.round(data[i,]/N,0)*N).astype(np.int32), lim_up)