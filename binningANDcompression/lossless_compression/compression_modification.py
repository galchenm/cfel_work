...
output = '/gpfs/cfel/group/cxi/scratch/2020/EXFEL-2019-Schmidt-Mar-p002450/scratch/galchenm/scripts_for_work/binningANDcompression/lossless_compression/test_on_single_image_results_all_levels'
...
def get_image_from_hdf5(path, dataset, index=0):
    file = h5py.File(path)
    ...
    if dataset.ndim == 2:
        ...
        raise ValueError("Dataset must have 2 or 3 dimensions")
    return dataset


def benchmark_image(image_name, image, algorithms):
...
def benchmark_single_images(images, algorithms):
    Path("single_image_benchmarks").mkdir(exist_ok=True)
    for (name, image) in images:
        ...
        df = benchmark_image(image_name, image, algorithms)
        df.to_csv(f"single_image_benchmarks/benchmark_{image_name}.csv")

        print(df)
        print()

        for (alg_name, alg_opts, extra_opts) in algorithms:
            space_ratio = df.loc[alg_name, "comp_ratio"]
            min_time_comp_alg = df.loc[alg_name, "comp_time_ratio"]
            min_time_decomp_alg = df.loc[alg_name, "decomp_time_ratio"]
            marker = d[alg_name]['marker']
            color = d[alg_name]['color']
            ax_comp.plot(
        ...
        #plt.savefig("plot_{}.pdf".format(image_name))
        plt.savefig(os.path.join(output, "plot_{}.png".format(image_name)))


def single_images():
    #yield "perkin elmer", imread("test_image_pe.tif")
    #yield "lambda", np.load("test_image_lambda.npy")
    #yield "analyzed", get_image_from_hdf5("test_image_analyzed.h5", "/sumimg")

    dir = "/asap3/fs-sc/gpfs/scdd01/2022/data/11015175/scratch_cc/"

    #for file in Path(dir).glob("**/Eiger_testdaten/*_data_000001.h5"):
    #    name = file.stem
    #    name = name[: -len("_data_000001")]
    #    yield name, get_image_from_hdf5(file, "/entry/data/data")

    for file in Path(dir).glob("**/Eiger16M/**/*.h5"):
        name = file.stem
        yield name, get_image_from_hdf5(file, "/data/data")

    for file in Path(dir).glob("**/AGIPD/**/*.h5"):
        name = file.stem
        yield name, get_image_from_hdf5(file, "/data/data")
...
algorithms = [
    ...
    ("lz4 + bs", bitshuffle_opts(), {}),
    ("blosc:lz4", blosc_opts(), {}),
    ("blosc:lz4 + bs", blosc_opts(shuffle="bit"), {}),
    ("blosc:blosclz", blosc_opts(complib="blosclz"), {}),
    ("blosc:blosclz + bs", blosc_opts(complib="blosclz", shuffle="bit"), {}),
    ("blosc:lz4hc", blosc_opts(complib="lz4hc"), {}),
    ("blosc:lz4hc + bs", blosc_opts(complib="lz4hc", shuffle="bit"), {}),
    
    (
        "blosc:zstd",
        blosc_opts(level=2, complib="zstd"),
        {"number": 10},
    ),
    (
        "blosc:zstd + bs",
        blosc_opts(level=2, complib="zstd", shuffle="bit"),
        {"number": 10},
    ),
    ("blosc:snappy", blosc_opts(complib="snappy"), {}),
    (
        "blosc:snappy + bs",
        blosc_opts(complib="snappy", shuffle="bit"),
        {},
    ),
...
#d = {i[0]:{'marker':marker, 'color':color} for (i,marker,color) in zip(algorithms, itertools.cycle(markers), itertools.cycle(colors)) } #for single image

d = {i[0]:{'marker':marker, 'color':color, 'comp_ratio':[], 'comp_time_ratio':[], 'decomp_time_ratio':[]} for (i,marker,color) in zip(algorithms, itertools.cycle(markers), itertools.cycle(colors)) } #for multiple

print(d)
benchmark_single_images(single_images(), algorithms)
