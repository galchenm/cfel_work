        ...
        if os.path.exists(os.path.join(path,FILE_NAME)):
            with open(os.path.join(path,FILE_NAME), 'r') as fileflow1:
                ...
                for line in fileflow1:
                    ...
                    if ">anisotropic</a></span></div>" in line:
                        readFlag = True
                        resolutionFlag = False
                        find_anisotropic = True
                        ...
                        pass
                if not(find_anisotropic):
                    print(f'There are no anisotropic section in {os.path.join(path,FILE_NAME)}')
...
if __name__ == "__main__":
    ...
    log_file = 'file-html.log'
    formatter = logging.Formatter('%(levelname)s - %(message)s')
    f_handler = logging.FileHandler(os.path.join(os.getcwd(), 'file-html.log'))
    logger.addHandler(f_handler)
    logger.info(f'{"path":<100}{"resolution":^10}{"CChalf":^10}{"a":^10}{"b":^10}{"c":^10}{"alpha":^10}{"beta":^10}{"gamma":^10}')
    print("Log file is {}".format(os.path.join(os.getcwd(), 'file-html.log')))
